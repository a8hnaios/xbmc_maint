...
define ("DEBUG", 0);
define ("INFO",  0);
define ("MOVIE_PATH", "/export/movies/Movies/*");
define ("DELETE_FILES", "Delete");
define ("CLEAN_UP_DB",  "Clean Up DB");

$dbs = array ("main_video90", "cassandra_video90", "nick_video90", "foreign_video90");
...
if ($submit == DELETE_FILES || $submit == CLEAN_UP_DB) {
   ...
   $q_del_files = "select strpath, strfilename ";
   $q_del_files.= "from files f, path p ";
...
if (isset($submit)) {
   ...
   if ($submit == DELETE_FILES) {
      ...
      while ($row = mysql_fetch_assoc($res_del_file)) {
         $del_files[$count] = substr($row['strpath'], stripos($row['strpath'], "/export/movies"));
...
for ($db_cnt=0; $db_cnt < count($dbs); $db_cnt++) {
   ...
   $q_in.= "from movie m, files f, path p ";
   $q_in.= "where m.idfile=f.idfile and f.idpath=p.idpath and ";
   if (isset($moviefilter)) {
      $q_in.= "strpath like '%/Movies/%".$moviefilter."%' order by path, file";
   } else {
      $q_in.= "strpath like '%/Movies/%' order by path, file";
   }

   # Select files that do not have a movie in the DB (i.e. orphans)
   $q_not_in = "select p.idpath, idfile, strPath as path, strfilename as file, playcount as watched ";
   $q_not_in.= "from path p, files f ";
   if (isset($moviefilter)) {
      $q_not_in.= "where strpath like '%/Movies/%".$moviefilter."%' ";
   } else {
      $q_not_in.= "where strpath like '%/Movies/%' ";
   }
   $q_not_in.= "and p.idpath=f.idpath ";
   $q_not_in.= "and f.idfile not in (select idfile from movie) order by path, file";
   ...
   $q_dirs.= "and strpath like '%/Movies/%' order by strpath";
   ...
   # Get all the orphan movies in the DB
   $count=0;
   while ($row = mysql_fetch_assoc($res_not_in)) {
      ...
      $files_not_in_db[$db_name][$count]['movie'] = $path.$file;
      $files_not_in_db[$db_name][$count]['watched'] = $row['watched'];
   ...
   while ($row = mysql_fetch_assoc($res_dirs)) {
      $path = substr($row['path'], stripos($row['path'], "/export/movies"));
...
#####################
$count=0;
$path = array("/export/movies/Movies/*");
...
for ($db_cnt = 0; $db_cnt < count($dbs); $db_cnt++) {
   ...
   for ($x = 0; $x < count($files_not_in_db[$db_name]); $x++) {
      ...
      if ($files_not_in_db[$db_name][$x]['other_dbs'] == "") {
         ...
         if (!in_array($movie, $files_on_disk)) {
...
<html>
<HEAD>
  <TITLE>XBMC Movies Clean Up</TITLE>
...
<FORM ID=xbmcmovies NAME=xbmcmovies METHOD=POST action="<?php echo $_SERVER['PHP_SELF']; ?>">

<h2>
XBMC Movie Management
...
<td>
<select name=DB ID=DB OnChange ="clear_all(); document.xbmcmovies.submit()">
...
<TD CLASS=SORT_NONEHDR> Genre/Movie </TD>
...
for ($x = 0; $x < count($files_not_in_db[$db_name]); $x++) {
   if ($files_not_in_db[$db_name][$x]['delete'] == 'Y') {
      ...
      $movie = substr($files_not_in_db[$db_name][$x]['movie'], strlen("/export/movies/Movies/"));
      $details = explode ("/", $movie);
      $genre = $details[0];
      $real_movie = substr($movie, strlen($genre)+1);
      $watched = $files_not_in_db[$db_name][$x]['watched'];
      if ($watched >= 1) {
         $watched = "Y";
      } else {
         $watched = "N";
      ...
      if ($old_genre != $genre) {
         ...
         print "<td bgcolor=#FAFF7E align=center> <input type=checkbox class='".$genre."_del' name='del_some' onClick=\"toggle_class(this)\" </td>";
         print "<td width=85% bgcolor=#FAFF7E><b>$genre</b></td>";
         print "</TR> \n";
         $row = 0;
      }
      ...
      print "<input type=checkbox class='".$genre."_del' name='del_all[]' value='$idfile'>";
      print "</td> ";
      print "<td width=85%>";
...
for ($x = 0; $x < count($files_not_in_db[$db_name]); $x++) {
   if ($files_not_in_db[$db_name][$x]['clean_db'] == 'Y') {
      ...
      if ($old_genre != $genre) {
         ...
         print "<td bgcolor=#FAFF7E align=center> <input type=checkbox class='".$genre."_clean' name='clean_some' onClick=\"toggle_class2(this)\" </td>";
         print "<td width=85% bgcolor=#FAFF7E><b>$genre</b></td>";
         print "</TR> \n";
         $row = 0;
      }
      ...
      print "<td align=center>";
      print "<input type=checkbox class='".$genre."_clean' name='clean_all[]' value='$idfile'>";
      print "</td> ";
      print "<td width=85%>";
